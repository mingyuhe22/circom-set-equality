...
const fs = require("fs");
const circuit_name = "set_equality";
const len = 100;
...
describe(`Test "${circuit_name}"`, function () {
    ...
    before(async () => {
        ...
        fs.writeFileSync(path.join(__dirname, `../build/${circuit_name}/circuit.circom`),
            `pragma circom 2.1.5;` + "\n" +
            `include "../../circuits/set_equality.circom";` + "\n" +
            "\n" +
            `component main = IsEqualSet(${len});`
        );
    });
    ...
    it("Wtns Check", async function () {
        ...
        var challenge = Math.floor(Math.random() * 1000).toString();
        var left = [];
        var right = [];
        for (var i = 0; i < len; i++) {
        ...
        const w = await circuit.calculateWitness({ left, right, challenge });
